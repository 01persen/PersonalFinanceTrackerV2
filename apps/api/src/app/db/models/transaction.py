...
class Transaction(Base, UUIDPKMixin, UserFKMixin, TimestampMixin):
    ...
    # Soft-delete column — populated by DELETE /transactions/:id (sub-0003-02).
    # The summary aggregator (sub-0003-04) filters on ``deleted_at IS NULL`` so
    # tombstoned rows never inflate income/expense totals or breakdowns. The
    # supporting ``ix_transactions_user_deleted_at`` index keeps the filter
