...
- epic **0005** sekarang **IN_PROGRESS** (sourced 2026-07-31, re-source
  ...
  - [x] sub-0005-06 — QA Integration + e2e test plan Epic 0005 → **DONE** 2026-08-02 ([GRE-62](https://multica/issues/GRE-62)), QA. QA PASS (Stage E) semua 4 Epic AC (a–d) + 4 high-risk area tests end-to-end. Backend regression full clean + FE `next build` hijau + mobile e2e Playwright 390×844 + DB migration upgrade + downgrade tested. `feat/sub-0005-06-fix-tz-as-of` last commit di `release/epic-0005` HEAD.
...
- v4.8 (2026-08-02) — Tech Leader: epic-0005 **Stage H created**. Stage 5
  ...
  last commit di `release/epic-0005` HEAD). Wake dari system hand-off
  comment (Stage G auto-progress trigger) — TL decide issue NOT
  actually complete (code belum merge ke `main`, parent belum flip
  ke `done`). Stage 6 (Stage H) created dengan 2 sub-task:
